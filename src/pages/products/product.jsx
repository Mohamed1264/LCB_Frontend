import React, { useState, useMemo, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { useTranslation } from "react-i18next";
import { useMe } from "../../hooks/useAuth";
import { useProducts, useDeleteProduct } from "../../hooks/useProducts";
import { useLayout } from "../../hooks/useLayout";
import Sidebar from "../../components/layout/Sidebar";
import Navbar from "../../components/layout/Navbar";
import Modal from "../../components/common/Modal";
import { PageLoader } from "../../components/common/Loading";
import PermissionGate from "../../routes/PermissionGate";
import {
  Trash2,
  Edit,
  Plus,
  Box,
  Search,
  DollarSign,
  LayoutGrid,
  Table,
  Package,
} from "lucide-react";

const IMAGE_BASE_URL = "http://127.0.0.1:8000/images/products/";

const Products = () => {
  const { t, i18n } = useTranslation();
  const navigate = useNavigate();
  const { data: meData, isLoading: isLoadingMe } = useMe();
  const companyId = meData?.user?.company?.id;

  const { data: products, isLoading: isLoadingProducts } =
    useProducts(companyId);

  const deleteMutation = useDeleteProduct();
  const { currentLang, getSidebarMargin } = useLayout();

  const [searchTerm, setSearchTerm] = useState("");
  const [selectedId, setSelectedId] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const [viewMode, setViewMode] = useState(
    localStorage.getItem("productsView") || "grid"
  );

  console.log(products);
  useEffect(() => {
    localStorage.setItem("productsView", viewMode);
  }, [viewMode]);

  const filteredProducts = useMemo(() => {
    if (!products) return [];
    return products.filter((product) => {
      const searchLower = searchTerm.toLowerCase();
      return (
        product.name?.toLowerCase().includes(searchLower) ||
        product.reference?.toLowerCase().includes(searchLower) ||
        product.category?.name?.toLowerCase().includes(searchLower)
      );
    });
  }, [products, searchTerm]);

  const handleDelete = () => {
    if (!selectedId) return;
    deleteMutation.mutate(
      { companyId, id: selectedId },
      {
        onSuccess: () => {
          setIsModalOpen(false);
          setSelectedId(null);
        },
      }
    );
  };

  if (isLoadingMe || isLoadingProducts) return <PageLoader />;

  return (
    <div className="flex bg-gray-50 min-h-screen font-sans" dir={currentLang === "ar" ? "rtl" : "ltr"}>
      <Sidebar />
      <Navbar />

      <main className={`flex-1 p-6 pt-24 transition-all duration-300 ${getSidebarMargin()}`}>
        {/* Header */}
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-xl font-black text-gray-800">
            {t("products.title")} ({filteredProducts?.length || 0})
          </h1>

          <div className="flex items-center gap-3">
            <PermissionGate permission="product_add">
              <button 
                onClick={() => navigate("/products/start")}
                className="flex items-center gap-2 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition"
              >
                <Plus size={18} /> {t("products.add")}
              </button>
            </PermissionGate>
          </div>
        </div>

        {/* Search */}
        <div className="relative mb-6">
          <Search
            className={`absolute ${currentLang === "ar" ? "right-3" : "left-3"} top-1/2 -translate-y-1/2 text-gray-400`}
            size={16}
          />
          <input
            type="text"
            placeholder={t("products.search_placeholder")}
            className={`w-full ${currentLang === "ar" ? "pr-10 pl-4" : "pl-10 pr-4"} py-2.5 bg-white border border-gray-100 rounded-xl text-sm focus:ring-2 focus:ring-orange-500 outline-none transition-all shadow-sm`}
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
        </div>
        
        <div className="flex items-center gap-2 justify-end mb-6">
          <button
            onClick={() => setViewMode("grid")}
            className={`px-3 py-1.5 text-xs font-bold rounded-lg transition flex items-center gap-1 ${
              viewMode === "grid"
                ? "bg-orange-600 text-white"
                : "text-gray-600 hover:bg-gray-50"
            }`}
          >
            <LayoutGrid size={14} />
            {t("products.view_grid")}
          </button>
          <button
            onClick={() => setViewMode("table")}
            className={`px-3 py-1.5 text-xs font-bold rounded-lg transition flex items-center gap-1 ${
              viewMode === "table"
                ? "bg-orange-600 text-white"
                : "text-gray-600 hover:bg-gray-50"
            }`}
          >
            <Table size={14} />
            {t("products.view_table")}
          </button>
        </div>

        {/* GRID VIEW */}
        {viewMode === "grid" ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {filteredProducts.length === 0 ? (
              <div className="col-span-full text-center py-12">
                <Box className="mx-auto text-gray-300 mb-3" size={48} />
                <p className="text-gray-500 font-medium">
                  {t("products.empty")}
                </p>
              </div>
            ) : (
              filteredProducts.map((product) => {
                const imageUrl = product.image ? IMAGE_BASE_URL + product.image : null;
                console.log(imageUrl);
                 

                return (
                  <div
                    key={product.id}
                    className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition overflow-hidden flex flex-col"
                  >
                    {/* IMAGE */}
                    <div className="relative h-48 bg-gray-100">
                      {imageUrl ? (
                        <img
                        src={imageUrl}
                        alt={product.name}
                        className="w-full h-full object-cover"
                      />) : (
                        <div className="w-full h-full bg-gray-100 flex items-center justify-center">
                          <Package size={100 } className="text-gray-400" />
                        </div>
                      )}

                      <span className="absolute top-3 right-3 bg-black/70 text-white text-xs font-bold px-3 py-1 rounded-full">
                        #{product.reference}
                      </span>
                    </div>

                    {/* CONTENT */}
                    <div className="p-4 flex flex-col flex-1">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <h3 className="font-black text-gray-800 text-sm">
                            {product.name}
                          </h3>
                          <p className="text-xs text-gray-400">
                            {product.category?.name}
                          </p>
                        </div>

                        <div className="flex gap-1">
                          <PermissionGate permission="product_edit">
                            <button 
                              onClick={() => navigate(`/products/edit/${product.id}`)}
                              className="p-1.5 text-blue-500 hover:bg-blue-50 rounded-md"
                            >
                              <Edit size={14} />
                            </button>
                          </PermissionGate>
                          <PermissionGate permission="product_delete">
                            <button
                              onClick={() => {
                                setSelectedId(product.id);
                                setIsModalOpen(true);
                              }}
                              className="p-1.5 text-red-500 hover:bg-red-50 rounded-md"
                            >
                              <Trash2 size={14} />
                            </button>
                          </PermissionGate>
                        </div>
                      </div>

                      <div className="flex items-center gap-2 text-orange-600 font-bold text-sm mb-3">
                        <DollarSign size={14} />
                        {product.price_min} - {product.price_max} {t("products.currency")}
                      </div>

                      <div className="text-xs text-gray-600 space-y-1 mb-3">
                      
                        <div>
                          <span className="font-semibold">{t("products.description")}</span>{" "}
                          {product.description}
                        </div>
                        <div>
                          <span className="font-semibold">{t("products.shipment")}</span>{" "}
                          {product.shipment?.reference}
                        </div>
                      </div>

                      <div className="flex flex-wrap gap-2 mt-auto pt-3 border-t border-gray-100">
                        <span
                          className={`text-[10px] font-bold px-2 py-1 rounded-md ${
                            product.is_local_product
                              ? "bg-green-50 text-green-600"
                              : "bg-blue-50 text-blue-600"
                          }`}
                        >
                          {product.is_local_product
                            ? t("products.local")
                            : t("products.imported")}
                        </span>

                        {product.is_rentable && (
                          <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-purple-50 text-purple-600">
                            {t("products.rentable")}
                          </span>
                        )}

                        </div>
                    </div>
                  </div>
                );
              })
            )}
          </div>
        ) : (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-600">
                    <tr className={`text-${currentLang === "ar" ? "right" : "left"}`}>
                    <th className="p-3 justify-start font-bold">{t("products.table.name")}</th>
                    <th className="p-3  font-bold">{t("products.table.reference")}</th>
                    <th className="p-3 font-bold">{t("products.table.category")}</th>
                    <th className="p-3 font-bold">{t("products.table.price")}</th>
                    <th className="p-3 font-bold">{t("products.table.type")}</th>
                    <th className="p-3 font-bold">{t("products.table.actions")}</th>
                    </tr>
              </thead>
              <tbody>
                {filteredProducts.length === 0 ? (
                  <tr>
                    <td colSpan="6" className="text-center py-8 text-gray-400">
                      {t("products.empty")}
                    </td>
                  </tr>
                ) : (
                  filteredProducts.map((product) => (
                    <tr
                      key={product.id}
                      className="border-t border-gray-100 hover:bg-orange-50 transition"
                    >
                      <td className="p-3 font-bold text-gray-800">
                        {product.name}
                      </td>
                      <td className="p-3 text-gray-500">
                        {product.reference}
                      </td>
                      <td className="p-3 text-gray-500">
                        {product.category?.name || "-"}
                      </td>
                      <td className="p-3 font-semibold text-gray-700">
                        {product.price_min} - {product.price_max} {t("products.currency")}
                      </td>
                      <td className="p-3">
                        <span
                          className={`text-xs font-bold px-2 py-1 rounded-md ${
                            product.is_local_product
                              ? "bg-green-50 text-green-600"
                              : "bg-blue-50 text-blue-600"
                          }`}
                        >
                          {product.is_local_product
                            ? t("products.local")
                            : t("products.imported")}
                        </span>
                      </td>
                      <td className="p-3">
                        <div className="flex gap-2">
                          <PermissionGate permission="product_edit">
                            <button 
                              onClick={() => navigate(`/products/edit/${product.id}`)}
                              className="p-1.5 text-blue-500 hover:bg-blue-50 rounded-md"
                            >
                              <Edit size={14} />
                            </button>
                          </PermissionGate>
                          <PermissionGate permission="product_delete">
                            <button
                              onClick={() => {
                                setSelectedId(product.id);
                                setIsModalOpen(true);
                              }}
                              className="p-1.5 text-red-500 hover:bg-red-50 rounded-md"
                            >
                              <Trash2 size={14} />
                            </button>
                          </PermissionGate>
                        </div>
                      </td>
                    </tr>
                  ))
                )}
              </tbody>
            </table>
          </div>
        )}
      </main>

      <Modal
        isOpen={isModalOpen}
        onClose={() => {
          setIsModalOpen(false);
          setSelectedId(null);
        }}
        onConfirm={handleDelete}
        title={t("products.delete.title")}
        confirmText={t("products.delete.confirm")}
        isLoading={deleteMutation.isPending}
      >
        <div className="text-center">
          <p className="text-sm text-gray-600">
            {t("products.delete.message")}{" "}
            <span className="font-bold text-red-600">
              "{products?.find((p) => p.id === selectedId)?.name}"
            </span>
            {t("products.delete.question")}
          </p>
        </div>
      </Modal>
    </div>
  );
};

export default Products;