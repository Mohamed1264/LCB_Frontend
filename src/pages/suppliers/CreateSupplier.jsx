...
const CreateSupplier = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(error);
      setErrorMessage({ title: t("supplier_create_error") || "خطأ", message: error.response?.data?.message || error.message });
        ...
        <div className={`pt-24 transition-all duration-300 ${getSidebarPadding()}`}>
          <div className="max-w-2xl mx-auto p-6">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">{t("supplier_create_success") || "تم إنشاء المورد"}</h2>
              <p className="text-gray-600 mb-8">{t("supplier_create_success_message") || "تم إضافة المورد بنجاح"}</p>
              <div className="flex gap-4">
                <button onClick={() => setShowSuccess(false)} className="flex-1 px-6 py-3 border border-gray-300 rounded-lg">{t("add_another") || "إضافة آخر"}</button>
      ...
      <div className={`pt-24 transition-all duration-300 ${getSidebarPadding()}`}>
        <div className="max-w-3xl mx-auto p-6">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">{t("supplier_create") || "إضافة مورد جديد"}</h1>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex gap-4">
                ...
                <button type="submit" className="flex-1 px-6 py-3 bg-orange-500 text-white rounded-lg">{t("create") || "إنشاء"}</button>
              </div>
            </form>
          </div>
        </div>
